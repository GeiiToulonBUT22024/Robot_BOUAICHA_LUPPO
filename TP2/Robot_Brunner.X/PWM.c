...
void PWMSetSpeed(float vitesseEnPourcents, int MOTOR) {
    if (MOTOR == 1) {
        ...
        if (vitesseEnPourcents < 0) {
            ...
            MOTEUR_GAUCHE_H_IO_OUTPUT = 0; //Mise à1 de la pin
        }
        if (vitesseEnPourcents > 0) {
            MOTEUR_GAUCHE_L_PWM_ENABLE = 1; //Pilotage de la pin en mode PWM
            MOTEUR_GAUCHE_L_IO_OUTPUT = 0; //Mise à1 de la pin
        ...
        if (vitesseEnPourcents < 0) {
            ...
            MOTEUR_DROIT_L_IO_OUTPUT = 0; //Mise à1 de la pin
            MOTEUR_DROIT_H_PWM_ENABLE = 0; //Pilotage de la pin en mode PWM
            MOTEUR_DROIT_H_IO_OUTPUT = 1; //Mise à1 de la pin
        }
        if (vitesseEnPourcents > 0) {
            MOTEUR_DROIT_L_PWM_ENABLE = 0; //Pilotage de la pin en mode PWM
            MOTEUR_DROIT_L_IO_OUTPUT = 1; //Mise à1 de la pin
            MOTEUR_DROIT_H_PWM_ENABLE = 1; //Pilotage de la pin en mode IO
            MOTEUR_DROIT_H_IO_OUTPUT = 0; //Mise à1 de la pin
        }
